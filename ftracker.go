...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case "Бег":
		...
		calories := RunningSpentCalories(action, weight, distance)
		return createMessage(trainingType, duration, distance, speed, calories)
	case "Ходьба":
		distance := distance(action)
		speed := meanSpeed(action, duration)
...
func createMessage(trainingType string, duration, distance, speed, calories float64) string {
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMsec, 2.0)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
